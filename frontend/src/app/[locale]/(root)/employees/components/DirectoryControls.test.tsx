...
vi.mock('@/components/filters/FilterSearchInput', () => ({
  FilterSearchInput: ({
    ...
    <div>
      ...
      <button type="button" data-testid="change-value" onClick={() => onChange('Luis')}>
        change
      </button>
      <button type="button" data-testid="search-undefined" onClick={() => onSearch()}>
        search undefined
      </button>
    </div>
  ),
...
describe('DirectoryControls', () => {
  ...
  it('updates query and calls onSearch when filter changes', () => {
    ...
    const { rerender } = render(<DirectoryControls onSearch={onSearch} searchValue="Ana" />)
