...
export default function NewRequestForm() {
  ...
  const selectedAbsenceType = absenceTypes.find((t: any) => t.id === watchAbsenceTypeId);
    ...
    <div className="max-w-2xl mx-auto py-6">
      <Card className="shadow-premium border-outline-variant/10">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                {form.formState.errors.absenceTypeId && (
                  <p className="text-sm text-destructive">{form.formState.errors.absenceTypeId.message}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Popover>
                    ...
                    >
                      ...
                      {watchStartDate ? format(watchStartDate, "PPP") : <span>{t('pickDatePlaceholder')}</span>}
                    </PopoverTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    ...
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {watchEndDate ? format(watchEndDate, "PPP") : <span>{t('pickDatePlaceholder')}</span>}
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0">
                      <Calendar
                        mode="single"
              ...
              {duration && duration > 0 && (
                <div className="p-3 bg-secondary/5 rounded-lg text-sm border border-secondary/10 flex items-center gap-3">
                  ...
                  <span>
                    <strong>{t('estimatedDuration')}:</strong> {duration} {t('workingDays')}
                  </span>
                </div>
              )}
              ...
              {selectedAbsenceType?.requiresHospitalInfo && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 border border-primary/20 bg-primary/5 rounded-md">
                  <div className="md:col-span-2">
                    <h4 className="text-sm font-semibold text-primary mb-2">{t('medicalAffidavitTitle')}</h4>
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="hospitalName">{t('hospitalNameLabel')} *</Label>
                    <Input 
                      id="hospitalName" 
        ...
        <CardFooter className="flex justify-between border-t border-outline-variant/10 p-6">
          <Button variant="ghost" onClick={() => router.back()} className="hover:bg-rose-50 hover:text-rose-600">
            {t('cancelButton')}
          </Button>
          <Button type="submit" form="leave-form" disabled={isSubmitting} className="bg-primary hover:bg-primary/90 shadow-md">
