...
const ModuleNavBarWrapper = ({
  ...
  selectedModule: NonNullable<ReturnType<typeof useNavigation>['selectedModule']>
}) => {
  ...
  useEffect(() => {
    if (previousModuleId.current === null || selectedModule.id !== previousModuleId.current) {
...
const Layout: FC<LayoutProps> = ({ children }) => {
  ...
  const hasSubmodules = selectedModule?.subModules && selectedModule.subModules.length > 0

  return (
    <div>
      <SidebarProvider defaultOpen={false} sidebarIconWidth='4rem'>
        {hasSubmodules && selectedModule && <ModuleNavBarWrapper selectedModule={selectedModule} />}
