...
vi.mock('next-auth/react', () => ({
  useSession: () => ({ data: { accessToken: 'token-123' }, status: 'authenticated' }),
}))
...
describe('usePaginatedQuery', () => {
  ...
  it('uses custom pageSize', () => {
    renderHook(() => usePaginatedQuery(MOCK_QUERY, { getPageInfo, pageSize: 25 }))
  ...
  it('nextPage does nothing when endCursor is null', () => {
    mockUseQuery.mockReturnValue({
      data: { items: { edges: [], pageInfo: { ...mockPageInfo, endCursor: null } } },
      loading: false,
      error: null,
      refetch: mockRefetch,
    })
  ...
  it('previousPage does nothing when startCursor is null', () => {
    mockUseQuery.mockReturnValue({
      data: { items: { edges: [], pageInfo: { ...mockPageInfo, startCursor: null } } },
      loading: false,
      error: null,
      refetch: mockRefetch,
    })
  ...
  it('uses custom fetchPolicy when provided', () => {
    renderHook(() =>
      usePaginatedQuery(MOCK_QUERY, { getPageInfo, fetchPolicy: 'network-only' })
    )
