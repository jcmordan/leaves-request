 ...
 * Consumes preloaded data via useSuspenseQuery or reads from cache.
 */
export default function DashboardContent() {
  ...
  const { data } = useSuspenseQuery(DASHBOARD_DATA_QUERY);
  
  // Example of using real data: 
  // In a real scenario, we'd have a specific "stats" query.
  // For now, we utilize the preloaded data structure.
  const vacationBalance = 14; 
  const sickLeaveBalance = 5;
  const recentRequests = (data as any)?.myRequests?.nodes || [];
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          { icon: Calendar, label: t('vacationBalance'), value: vacationBalance, sub: t('daysAvailable') },
          { icon: AlertCircle, label: t('sickLeave'), value: sickLeaveBalance, sub: t('daysAvailable') },
        ...
        <div className="space-y-4">
          {recentRequests.length > 0 ? (
            recentRequests.map((req: any) => (
              <div key={req.id} className="bg-surface-container-low/30 rounded-lg p-4 border border-outline-variant/10 flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-primary">{req.absenceType?.name || 'Untitled'}</h4>
