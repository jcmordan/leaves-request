    ...
    () =>
      Object.entries(state).flatMap(([module, actions]) =>
        actions
          ...
          .map(action => {
            const patientName = action.metadata['PatientName'] || 'Unknown Patient'
            const scheduledTime = action.metadata['ScheduledStartTime'] || action.createdAt
            ...
            <div className='flex flex-col gap-1'>
              {notifications.map(notification => (
                <Item
                  key={notification.id}
                  className={cn(
                    notification.isUrgent && 'bg-red-50 hover:bg-red-100/50 border rounded-none',
                    'pb-0'
                  )}
                >
                    ...
                    <ItemHeader>
                      ...
                      <ItemActions>
                        <Button
                          ...
                          onClick={() => handleDismiss(notification.id, notification.module)}
                        >
